...
for _ in range(5):
    ...
    if flip == 0:
        fishes.add(Fish(random.randint(-SCREEN_WIDTH, -TILE_SIZE),
                        random.randint(FONT_SIZE, SCREEN_HEIGHT - RECTANGLE_HEIGHT - 2*TILE_SIZE)))
    elif flip == 1:
        fishes.add(Fish(random.randint(SCREEN_WIDTH + TILE_SIZE, 2*SCREEN_WIDTH),
                        random.randint(FONT_SIZE, SCREEN_HEIGHT - RECTANGLE_HEIGHT - 2*TILE_SIZE)))
...
while running:
    ...
    for fish in fishes:
        if fish.rect.x < 0:
            fish.
            fish.update_forward()
        elif fish.rect.x > SCREEN_WIDTH:
            fish.draw_backward(background)
            fish.update_backward()
#    fishes.update()
